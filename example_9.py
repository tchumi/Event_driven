...
# 1. Add a new event type called NEW
# 2. prioritization of events
# 3. filtering of events
# 4. middleware functions to be applied to events before and after filtering
# 9. tracking of processed events in a logging system
...
def filter_event(event: Event) -> bool:
    if (
        event.event_type in filteredType
        or event.priority in filtedPriority
        or event.event_source in filteredSource
    ):
        return True
    else:
        return False
...
async def produce_event(queue: EventQueue):
    while True:
        ...
        event_source: Source = random.choice([source for source in Source])
        ...
        print(
            f"Produced: {event_type} priority: {event_priority} source: {event_source}"
        )
        ...
        if not filter_event(event):
            ...
            print(
                f"Filtered: {event_type} priority: {event_priority} source: {event_source}"
            )
